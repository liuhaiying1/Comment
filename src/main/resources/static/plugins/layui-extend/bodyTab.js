...
layui.define(['element', 'layer', 'jquery'], function (drdrdr) {
    ...
    Tab.prototype.render = function () {
        ...
        //res的值即为menu.json里面的数据，根据返回的dd数据中的groupId拆分res数据，不同的角色看到不同的菜单
        ajaxSettings.success = function (res) {
            ...
            if (res.code == 200) {
                ...
                com.ajax({
                    ...
                    success: function (dd) {
                        var navB = [];
                        if (dd.groupId == 3) {          //当前用户为管理员
                            navB.push(res.data[0]);
                            navB.push(res.data[1]);
                            navB.push(res.data[3]);
                        } else if (dd.groupId == 2) {   //当前用户为商家
                            navB.push(res.data[0]);
                            navB.push(res.data[1]);
                            ...
                            com.ajax({
                                ...
                                success: function (rr) {
                                    if (rr.userid != 0) {
                                        res.data[2].children.push({
                                            "title": "商家信息管理",
                                            "icon": "&#xe62a;",
                                            "href": "/bussiness/editform",
                                            "spread": "false",
                                            "children": []
                                        });
                                    }
                                    navB.push(res.data[2]);
                                }
                            })
